...
class LarvaeAge:
    ...
    def fetching_data(self):
        df = pd.read_excel('/Users/mohitjain/Desktop/Msc Project on Machine Learning/GUI model for Offline Users/Dataset.xlsx')
    ...
    def path_view(self, length, weight, larvelStage, temperature):
        ...
        viz = dtreeviz(regressor,
                ...
                show_just_path=True)
        viz.save(f"uploaded_images/path_view{length}-{weight}-{larvelStage}-{temperature}.svg")
        # graph = graphviz.Source(viz, format="png") 
        # graph.save(f"uploaded_images/path_view{length}-{weight}-{larvelStage}-{temperature}.png")
    ...
    def tree_view(self, length, weight, larvelStage, temperature):
        ...
        viz.save(f"uploaded_images/tree_view{length}-{weight}-{larvelStage}-{temperature}.svg")
